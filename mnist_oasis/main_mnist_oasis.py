...
from dataloaders import MNIST_OASIS_DL
...
class Solver():
    def __init__(self, model, linear_cca, outdim_size, epoch_num, batch_size, learning_rate, reg_par, device=torch.device('cpu')):
        ...
        wandb.init(project = 'DCCA_mnist_oasis', entity = 'asenellart', config = {'batch_size' : batch_size,
...
if __name__ == '__main__':
    ...
    epoch_num = 10
    batch_size = 800
    train_loader,test_loader = MNIST_OASIS_DL().getDataLoaders(batch_size=10)

    item = train_loader.dataset.data[1][0]
    print(item)
